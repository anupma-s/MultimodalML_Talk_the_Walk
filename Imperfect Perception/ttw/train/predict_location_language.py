...
def eval_epoch(loader, guide, opt=None):
    ...
    for batch in loader:
        g_out = guide.forward(batch[1], add_rl_loss=True)
        accs += g_out['acc']
        total += 1
...
def get_mean_T(loader, guide):
    ...
    for batch in loader:
        input_emb = guide.embed_fn(batch[1]['utterance'])
        hidden_states, _ = guide.encoder_fn(input_emb)

        batch_sz = batch[1]['utterance'].size(0)
        last_state_indices = batch[1]['utterance_mask'].sum(1).long() - 1
