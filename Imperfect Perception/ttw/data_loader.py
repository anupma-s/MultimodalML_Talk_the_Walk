...
class TalkTheWalkEmergent(Dataset):
    ...
    def __init__(self, data_dir, set, goldstandard_features=True, resnet_features=False, fasttext_features=False, T=2):
        ...
        self.map = Map(data_dir, neighborhoods, include_empty_corners=True, imperf=True)
        self.map2 = Map(data_dir, neighborhoods, include_empty_corners=True, imperf=False)
        self.T = T
        ...
        if goldstandard_features:
            ...
            self.feature_loaders2['goldstandard'] = GoldstandardFeatures(self.map2)
        ...
        for config in self.configs:
            for a in all_possible_actions:
                ...
                obs2 = {k: list() for k in self.feature_loaders2.keys()}
                actions = list()
                ...
                for p in range(self.T + 1):
                    ...
                    for k, feature_loader in self.feature_loaders2.items():
                        obs2[k].append(feature_loader.get(neighborhood, loc))
                ...
                for k in self.feature_loaders2.keys():
                    self.data2[k].append(obs2[k])
                ...
                self.data2['actions'].append(actions)
                landmarks, label_index = self.map2.get_landmarks(neighborhood, boundaries, target_loc)
                self.data2['landmarks'].append(landmarks)
                self.data2['target'].append(label_index)
                ...
                if msg['id'] == 'Tourist':
                    ...
                    if act_id >= 0:
                        ...
                        if orientation_aware:
                            ...
                            obs_memory2.append(self.feature_loader2.get(neighborhood, new_loc))
                        else:
                            if act == 'ACTION:FORWARD':  # went forward
                                ...
                                obs_memory2.append(self.feature_loader2.get(neighborhood, loc))
                        ...
                        landmarks, tgt = self.map2.get_landmarks(config['neighborhood'], boundaries, loc)
                        self.data2['landmarks'].append(landmarks)
...
class Map(object):
    ...
    def __init__(self, data_dir, neighborhoods, include_empty_corners=True, imperf=False):
        ...
        for neighborhood in neighborhoods:
            ...
            if imperf:
                ...
                self.landmarks[neighborhood] = json.load(open(os.path.join(data_dir, neighborhood, "map_original.json")))
