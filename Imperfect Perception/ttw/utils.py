...
def get_collate_fn(cuda=True):
    def _collate_fn(data):
        ...
        for k in data[0][0].keys():
            k_data = [data[i][0][k] for i in range(len(data))]
        ...
        for k in data[0][1].keys():
            k_data = [data[i][1][k] for i in range(len(data))]
            if k in ['textrecog', 'landmarks']:
                batch2[k], _ = list_to_tensor(k_data)
            if k in ['goldstandard', 'actions']:
                batch2[k], batch2[k + '_mask'] = list_to_tensor(k_data)
            if k == 'utterance':
                batch2['utterance'], batch2['utterance_mask'] = list_to_tensor(k_data)
            if k in ['target']:
                batch2[k] = torch.LongTensor(k_data)
            if k in ['resnet', 'weight']:
                batch2[k] = torch.FloatTensor(k_data)
            if k == 'fasttext':
                batch2[k], _ = list_to_tensor(k_data, tensor_type=torch.FloatTensor)
        return to_variable((batch,batch2), cuda=cuda)
    return _collate_fn

def get_collate_fn2(cuda=True):
    def _collate_fn2(data):
        batch = dict()
...
def list_to_tensor(arr, pad_value=0, tensor_type=torch.LongTensor):
    ...
    val_tensor = tensor_type(*dims).fill_(pad_value)
